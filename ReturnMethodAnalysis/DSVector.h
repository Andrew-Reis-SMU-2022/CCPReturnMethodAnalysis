...
public:
    ...
    DSVector(const T& elem, int numcpys) : DSVector<T>(numcpys) {
        cout << "Overloaded Constructor Called\n";
        for (int i = 0; i < numcpys; i++) {
            DSVector::append(elem);
        }
    }

    DSVector(const T& elem, int numcpys, chrono::high_resolution_clock::time_point& t) : DSVector<T>(numcpys) {
        cout << "Overloaded Constructor Called\n";
        for (int i = 0; i < numcpys; i++) {
            DSVector::append(elem);
        }
    ...
    DSVector(const initializer_list<T>& iList) {
    ...
    DSVector<T>& operator=(DSVector<T>&& rhs) {
    ...
    DSVector<T>& operator=(const initializer_list<T>& iList) {
		this->_size = iList.size();
    ...
    DSVector<T> operator+(const DSVector<T>& rhs) const{
		DSVector returnVec(*this);
    ...
    T& at(int index) const {
		if (index >= this->_size || index < -1 * this->_size) {
            throw out_of_range("DSVector::at Index Out of Bounds...");
		}
		if (index >= 0) {
    ...
    T& operator[](int index) const{
		return DSVector::at(index); // call at() instead of rewriting the same function
	}

    bool operator==(const DSVector<T>& rhs) const{
	...
	void resize(int newCap) {
		if (newCap < 0) {
            throw out_of_range("DSVector::resize Cannot allocate a vector of negative capactiy...");
		}
        this->cap = newCap;
    ...
    DSVector<T> slice(int lowerBound, int upperBound) const {
		if (upperBound >= this->_size || upperBound < -1 * this->_size) {
            throw out_of_range("DSVector::slice upper bound out of range");
		}
		...
		if (lowerBound > upperBound) {
            throw out_of_range("DSVector::slice lowerbound > upperbound");
		}
		
        DSVector<T> returnVec(upperBound - lowerBound + 1);
	...
	void remove(int index) {
		if (index >= this->_size || index < -1 * this->_size) {
            throw out_of_range("DSVector::Remove index out of bounds");
		}
		if (index < 0) {
    ...
    void insert(T elem, int index) {
        if (index > this->_size || index < -1 * this->_size) {
            throw out_of_range("DSVector::insert index out of bounds");
        }
        if (index < 0) {
    ...
    void insertInOrder(T elem) {
        ...
        DSVector::insert(elem, insertionIndex);
    }
